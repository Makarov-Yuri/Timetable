...
class ScrollSnapForTimetable{
	...
	//Массив в соответствии с которым будет происходить оценка существования блоков
	ArrayOfConditionsForExistOfBlocks=[];
	...
	//необходимые для создания объекта
		...
		this.ArrayOfConditionsForExistOfBlocks=ConfigureObject.ArrayOfConditionsForExistOfBlocks;
   ...
   //Объявляем методы класса
	...
	getArrayOfActiveChildrenBlocks(){//Функция для получения массива активных блоков
		...
		for(let i=0; i<=this.Container.children.length-1;i++){
			...
			if(/*this.Container.children[i].style.display!='none'*/this.ArrayOfConditionsForExistOfBlocks[i]==true){
	...
	StartAnimation(NumberOfChildrenBlock,ScrollDirection){
		...
		//Производим мутацию, чтобы показать, что каледрарь начал двигаться
		//store.state.AnimationOfCalendarIsPlaying=true;
		...
		let RunAnimation=()=>{
			...
			//Если анимации разрешается идти, то выполняем код дальше(выполняем следующую итерацию)
			if(this.AnimationCanHappening==true){
				...
				if(
					...
					this.AnimationIsHappening=false;
					//Производим мутацию, чтобы показать, что каледрарь начал двигаться
					//store.state.AnimationOfCalendarIsPlaying=false;
